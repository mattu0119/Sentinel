# Sentinel コンテンツ管理レポジトリ

このレポジトリは Microsoft Sentinel に展開するコンテンツを管理するレポジトリです。

## Sentinel レポジトリでサポートされるリソース

Microsoft Sentinel のレポジトリ連携では、以下のカスタムコンテンツがサポートされます。
...
レポジトリ内コンテンツへの更新は接続先ワークスペースに同期され、ポータル側で行った同一コンテンツへの変更は上書きされます。
そのため、このレポジトリを接続済みワークスペースのカスタムコンテンツに対する単一の信頼できるソースとして扱います。
